...
const getTypeByLang = (typeKey) => {
  ...
  return typeMap[typeKey] || typeMap.backpack
}
...
const backpackItems = computed(() => {
  const targetType = getTypeByLang('backpack')
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsClothingView] backpackItems - targetType: ${targetType}, count: ${filtered.length}, total items: ${itemsData.value.length}`)
  }
  return filtered
})
