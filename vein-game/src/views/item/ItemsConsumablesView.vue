...
const getTypeByLang = (typeKey) => {
  ...
  return typeMap[typeKey] || typeMap.alcoholDrugs
}
...
const alcoholDrugsItems = computed(() => {
  const targetType = getTypeByLang('alcoholDrugs')
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsConsumablesView] alcoholDrugsItems - targetType: ${targetType}, count: ${filtered.length}, total items: ${itemsData.value.length}`)
  }
  return filtered
})
