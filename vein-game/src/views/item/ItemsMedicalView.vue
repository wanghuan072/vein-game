...
const getTypeByLang = (typeKey) => {
  ...
  return typeMap[typeKey] || typeMap.bandages
}
...
const bandagesItems = computed(() => {
  const targetType = getTypeByLang('bandages')
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsMedicalView] bandagesItems - targetType: ${targetType}, count: ${filtered.length}, total items: ${itemsData.value.length}`)
  }
  return filtered
})
