...
const getTypeByLang = (typeKey) => {
  ...
  return typeMap[typeKey] || typeMap.basicTools
}
...
const basicToolsItems = computed(() => {
  const targetType = getTypeByLang('basicTools')
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsToolsView] basicToolsItems - targetType: ${targetType}, count: ${filtered.length}, total items: ${itemsData.value.length}`)
  }
  return filtered
})
