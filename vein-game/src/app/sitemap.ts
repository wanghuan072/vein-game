...
const contentLastModified = (entry: { publishDate?: string; updatedDate?: string }) => {
  const date = entry.updatedDate ?? entry.publishDate
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  for (const locale of ['en', 'de'] as Locale[]) {
    ...
    for (const guide of getGuides(locale)) entries.push({ url: new URL(localizePath(locale, `/vein-guides${guide.addressBar}`), siteConfig.domain).toString(), lastModified: contentLastModified(guide), changeFrequency: 'monthly', priority: 0.8 })
    for (const wiki of getWiki(locale)) entries.push({ url: new URL(localizePath(locale, `/vein-wiki${wiki.addressBar}`), siteConfig.domain).toString(), lastModified: contentLastModified(wiki), changeFrequency: 'monthly', priority: 0.7 })
  }
  return entries
}
